...
Mat& TJACC(Mat& source, Mat &refIm, Mat &dest, int WinWidth)
{
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++)
		{
			...
			double varCFval = VarCF.at<float>(o_y,o_x);                  
			double varDFval = VarDF.at<float>(o_y,o_x);                  
			double afa = VarF.at<float>(o_y,o_x);                        

			uchar FPix = F.at<uchar>(o_y, o_x);                         
			uchar srcPix  = source.at<uchar>(o_y,o_x);                   
			uchar refPix  = refIm.at<uchar>(y, x);                       
			...
			for (int j = -num_neighbors; j <= num_neighbors; j++)
			{
				...
				for (int i = -num_neighbors; i <= num_neighbors; i++)
				{
					...
					nsrcPix = source.at<uchar>(r_y, r_x);            
					nrefPix = refIm.at<uchar>(r_ys, r_xs);           
